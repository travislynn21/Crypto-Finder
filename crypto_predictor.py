...
def analyze_data_with_ml(data):
    ...
    # For ML, we need a target variable. Let's define a "winner" as a crypto that has a positive 24h change.
    df['is_winner'] = (df['price_change_percentage_24h'] > 0).astype(int)

    # Drop rows with NaN values that might have been generated by indicators
    df.dropna(inplace=True)

    if df.empty or df['is_winner'].nunique() < 2:
        print("Not enough data to train the model. Falling back to the old analysis method.")
        return analyze_data(data)

    # Define features (X) and target (y)
    features = ['market_cap', 'total_volume', 'sma_7', 'sma_30', 'rsi', 'macd', 'macd_signal', 'volatility', 'sharpe_ratio']
    X = df[features]
    y = df['is_winner']
    ...
    # Get the best crypto based on ML prediction
    # We want a predicted winner with the highest probability
    df['prediction_proba'] = model.predict_proba(X)[:, 1]

    potential_winners = df[df['prediction'] == 1]

    if not potential_winners.empty:
        best_crypto = potential_winners.loc[potential_winners['prediction_proba'].idxmax()]
    else:
        # Fallback to the original logic if no winner is predicted
